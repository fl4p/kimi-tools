...
- **Edit first; don't fight the environment.** Once you've localized the bug, make the source edit — do *not* spend your budget installing dependencies, setting up a virtualenv, or building a reproduction before editing. If `pip install` / a build / dependency setup fails once, stop retrying it and write the fix from reading the code; verify only if the environment already works. Burning every turn on environment wrangling and never editing is the worst possible outcome — an unverified-but-correct edit beats a perfectly-set-up environment with no edit.
